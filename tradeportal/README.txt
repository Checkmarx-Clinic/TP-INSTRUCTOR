...
copy dist/tradeportal.war to ${TOMCAT_HOME}/webapps
NOTE: if you do not deploy via the .war you will additionally need to copy context.xml to ${TOMCAT_HOME}/conf/Catalina/localhost/tradeportal.xml
start tomcat
browse to http://localhost:8080/tradeportal
login with admin/admin, eddie/eddie, gary/gary or another profile defined in create_db.sql


## REDEPLOYING ##
stop tomcat ()
delete ${TOMCAT_HOME}/webapps/tradeportal (delete the whole directory)
delete ${TOMCAT_HOME}/webapps/tradeportal.war
...
copy dist/tradeportal.war to ${JBOSS_SERVER_DIR}/deploy
copy etc/jboss4-support/tradeportal-ds.xml to ${JBOSS_SERVER_DIR}/deploy
...
6. Edit web.xml in tradeportal/WEB-INF, add the following to <security-constrain> section
